...
class EpisodicBuffer():
	...
	def read(self):
		self.memories=np.zeros((self.capacity,self.dim))
		self.mask=np.zeros((self.capacity,))
		if len(self.buffer)>0:
			self.memories[:len(self.buffer),:]=np.array(self.buffer)
			self.mask[:len(self.buffer)]=1
		return self.memories,self.mask 
...
class BattleshipEnv(gym.Env):  
	...
	def __init__(self,rules='chain',n_board=7,hold_out=0,permute=0,pretrain=0,render_img=0,memsize=40):
		...
		#s(t), a(t), s(t+1)
		self.memdim=n_board*n_board+n_board*n_board+n_board*n_board
		self.memsize=memsize 

		self.memory_buffer=EpisodicBuffer(self.memsize,self.memdim)
		...
		if self.render_img:
			...
			self.observation_space=Dict({
				'state':Box(low=-1, high=1, shape=(n_board*n_board+n_board*n_board+1,), dtype=np.int8),
				'memory':Box(low=-1,high=1,shape=(self.memsize,self.memdim),dtype=np.int8),
				'memory_mask':Box(low=0,high=1,shape=(self.memsize,),dtype=np.int8)
			})
	...
	def step(self, action):
		...
		curr_memory,curr_mask=self.memory_buffer.read() 
		obs_dict={'state':obs_array,'memory':curr_memory,'memory_mask':curr_mask}
		...
		#Push previous observation, current action, current observation onto memory buffer for next step. 
		#print("Pushing ",prev_board,self.prev_action,obs)' 
		self.memory_buffer.push(np.concatenate((prev_board,self.prev_action,obs.flatten())))
		#print(self.board)

		return obs_dict, reward, done, {'valid_actions':self.valid_actions}
...
def register_small_env(env_id,rules,n_board=7,hold_out=0,permute=0,max_episode_steps=60,pretrain=0,render_img=0,memsize=40):  
	gym.envs.register(id=env_id, entry_point=BattleshipEnv, max_episode_steps=max_episode_steps,kwargs={'rules':rules,'n_board':n_board,'hold_out':hold_out,'permute':permute,'pretrain':pretrain,'render_img':render_img,'memsize':memsize})  
