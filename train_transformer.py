...
hyperparams_dict=pickle.load(open('data/hyperparams_transformer_ppo_'+rules+'.pkl','rb'))
...
pk=dict(
        ...
        features_extractor_kwargs=dict(attn_dim=attn_dim,num_layers=num_layers,num_heads=num_heads,conv=conv,dropout=dropout)
    )
...
if __name__=='__main__':
    ...
    print("Saving") 
    model.save("/scratch/gpfs/sreejank/transformer_models/ppo_"+rules+"_metalearning_rep2e6.zip")
    print('loading')
    ...
    np.save('data/raw_choices_transformer_agent2e6_'+str(rules)+'.npy',np.asarray(raw_choices_total))  
    np.save('data/raw_performance_transformer_agent2e6_'+str(rules)+".npy",raw_performance) 
