"""
Code to generate metamer task distributions from a given abstraction (supply name with command line argument)
"""
import torch
...
class Net(torch.nn.Module):
	def __init__(self,S=7):
		...
		#self.fc4 = torch.nn.Linear(S*S, S*S)
		#self.fc5 = torch.nn.Linear(S*S, S*S)
...
class Net(torch.nn.Module):
	def __init__(self,S=7):
		...
		self.conv2=torch.nn.Conv2d(16,16,3,stride=1)
		self.fc1=torch.nn.Linear(144,72)
		self.fc2=torch.nn.Linear(72,49)
		self.fc3=torch.nn.Linear(49,49)
	def forward(self,x):
		x=torch.reshape(x,(-1,1,S,S))
		x=F.relu(self.conv1(x))
		x=F.relu(self.conv2(x))
		#print(x.shape)
		x=torch.reshape(x,(-1,144))
		x=F.relu(self.fc1(x))
		x=F.relu(self.fc2(x))
		x=self.fc3(x)
		return F.sigmoid(x)
